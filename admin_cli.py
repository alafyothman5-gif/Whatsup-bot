...
def products_cmd(args):
    ...
    for p in products:
        ...
        print(f"- {p.get('id')} | {p.get('category','')} | {p.get('name')} | {p.get('price')} | {p.get('stock')}")
        if aliases:
...
def pending_cmd(args):
    ...
    for item in pending:
        ...
        if vr:
            print(f"  vision: {vr.get('provider')} | {vr.get('product_name')} | conf={vr.get('confidence')} | reason={vr.get('strict_reason')}")
    print()
...
def add_product_cmd(args):
    ...
    new_product = {
        ...
        "quantity": args.quantity.strip() if args.quantity else "",
        "expiry_date": args.expiry_date.strip() if args.expiry_date else "",
    ...
    for i, p in enumerate(products):
        if p.get("id") == product_id:
            ...
            for a in old_aliases:
                if a not in new_product["aliases"]:
                    new_product["aliases"].append(a)
...
def resolve_cmd(args):
    ...
    found_pending = next((x for x in pending if x.get("id") == pending_id), None)
    found_product = next((x for x in products if x.get("id") == product_id), None)
    ...
    if not alias:
        ...
        if not alias:
            vr = found_pending.get("vision_result") or {}
...
def main():
    ...
    p_add.add_argument("--quantity", default="")
    p_add.add_argument("--expiry-date", default="")
